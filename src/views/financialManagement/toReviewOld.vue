      ...
      return {
        columns4: [
          ...
          },
          {
            ...
            key: 'operating',
            render (row, column, index) {
              return `<i-button v-if="${row.status}==0||${row.status}==1" type="text" size="small" @click="batchReview(${index})">同意</i-button>
                      <i-button v-if="${row.status}==0||${row.status}==1"type="text" size="small" @click="todoWithRefuse(${index})">拒绝</i-button>
      ...
      batchReview(index){
        ...
        if (/^[0-9]+$/.test(index)) {
          ...
          if (this.multiply.length == 0) {
            ...
            this.$Modal.confirm({
              title: '批量审核',
              content: `<p style="font-size: 16px;padding:10px 0;">您确定批量审核？</p>`,
              onOk: function () {
                var ids = "";
